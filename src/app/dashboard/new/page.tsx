...
import { Link as LinkIcon, FileText, AlertTriangle, ArrowRight, Save, Swords, ChevronLeft, X } from "lucide-react";
            ...
            {parsedCards.map((card) => (
              <div key={card.id} className={`flex flex-col md:flex-row md:items-center justify-between gap-4 p-3 border-l-4 transition-colors rounded-r-sm shadow-sm ${card.isStaple ? "bg-cyan-950/10 border-cyan-800 hover:bg-cyan-950/30" : "bg-black/40 border-[#1c1611] hover:border-[#8a7b6b]/60"}`}>
                <div className="flex items-center gap-4 flex-1 min-w-0">
                  <span className="text-[#8a7b6b] font-mono font-bold text-sm w-6 shrink-0">{card.qty}x</span>
                  <div className="flex flex-col truncate">
                    <button onClick={() => setPreviewCard(card.name)} className="text-[#e8e0d5] font-bold text-sm text-left truncate hover:text-cyan-400 transition-colors">{card.name}</button>
                    <span className="text-stone-500 text-[10px] uppercase tracking-widest mt-0.5 truncate">{card.type}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between md:justify-end gap-6 w-full md:w-auto mt-2 md:mt-0">
                  <div className="flex gap-1 shrink-0">
                    {card.colors.length > 0 ? card.colors.map(c => <ManaIcon key={c} symbol={c} />) : <ManaIcon symbol="C" />}
                  </div>
                  <div className="flex gap-4 shrink-0 bg-[#050308] p-2 border border-[#1c1611] rounded-sm">
